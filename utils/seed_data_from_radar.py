...
def radar_fix_json(text):
    ...
    def replacement(matchObj):
        ...
        try:
            ...
            print(f"Warning: Found match in replacement regex: {matchStr}; replacement value not availble; keeping string as-is")
            return matchStr
...
def cwms_office_symbols():
    ...
    offices_response = requests.get(
        'http://cwms-data.usace.army.mil/cwms-data/offices?format=json'
    ).json()
...
def locations_for_office_symbols(office_symbols):
    ...
    for f in office_symbols:
        office_locations = requests.get(
            f'http://cwms-data.usace.army.mil/cwms-data/locations?&office={f}&name=@&location_kind=PROJECT&format=json'
        )
        try:
            _j = office_locations.json()
            _j_locations = _j['locations']['locations']
            locations += _j_locations
            print(f"Successfully Extracted {len(_j_locations)} Locations for Office Symbol: {f}")
        except:
            try:
                ...
                print(f"ERROR: COULD NOT CONVERT API RESPONSE TO JSON FOR OFFICE SYMBOL: {f}")
...
if __name__ == "__main__":
    ...
    print(f"Running Data Extraction for Specified Office Symbols: \n\t{OFFICE_SYMBOLS}\n")
    ...
    for idx, l in enumerate(locations):
        ...
        if _public_name is None:
            ...
            _entry += quote(apostrophe_doubleup(_public_name)) + ","
        # Slug
    ...
    with open(OUTFILE, "w") as f:
        ...
        f.write("\n\n")
        f.write(f'INSERT INTO location (office_id, name, public_name, slug, geometry, kind_id) VALUES\n')
...
for k in unknown_offices_map.keys():
    print(f"Office '{k}' is not recognized as a Corps office; {unknown_offices_map[k]} corresponding locations were not loaded;")
